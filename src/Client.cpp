...
int main(int argc, char **argv)
{
	...
	clientSock = socket(AF_INET, SOCK_DGRAM, 0); //Internet flag, UDP protocol
	if(clientSock==-1)
    {
		ROS_WARN("Socket Establishment Failed!");
    }
	...
	const int opt = 1;
	int nb = setsockopt(clientSock, SOL_SOCKET, SO_BROADCAST, (char *)&opt, sizeof(opt)); // Broadcast Mode
	if(nb==-1)
	{
		ROS_WARN("Broadcast Set Failed!");
	}
	...
	/*int* ptr = NULL;
	GThread* udp_thread;
	udp_thread=g_thread_new("", udp_send, (void *)ptr);
	*/
	ros::spin();
